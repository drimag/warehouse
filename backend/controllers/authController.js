...
exports.register = async (req, res) => {
  try {
    ...
    if (!fullName || !email || !employeeId || !password) {
      return res
        .status(400)
        .json({ error: "All registration parameters are strictly required." });
    }

    // 2. Query database index to prevent duplicate account creation
    const existingUser = await userModel.findByEmailOrEmployeeId(
      email,
      employeeId,
    );
    if (existingUser) {
      return res.status(409).json({
        error:
          "A node account with this email address or Employee ID already exists inside the system.",
      });
    }
    ...
    return res
      .status(500)
      .json({
        error: "Internal system fault detected during user write sequence.",
      });
  }
};
...
exports.login = async (req, res) => {
  try {
    ...
    if (!user || !(await bcrypt.compare(password, user.password_hash))) {
      return res
        .status(401)
        .json({ error: "Invalid email credentials or password" });
    }
